import {
  ...
  export function ProductDetail1() {
    ...
    try{
        setError(false)
        setLoading(true)
       let response = await axios.get (`http://localhost:4000/products/4 `)
    ...
    return (
        ...
        {/* รูปใหญ่============================================================================================= */}
         {datas.map((value,index)=>{
            return(
                <Box key={index} boxShadow ="lg" h ={[100,300,500]} w ={[100,300,600]} mt ="175" ml ={[30,70,100]} > 
          ...
          {datas.map((value, index)=>{
           ...
           <Stack spacing={2}>
            ...
            <Text fontSize={{base:'10px',sm:'20px', md:'20px', lg:'30px'}}> {value.name} </Text>
            ...
            <Flex position="absolute" ml="150" mt="2.5">  <Button  borderColor="white" size="xs" variant="outline"> <Text fontSize={[5,10,12]}>{value.review}</Text> </Button>  </Flex> 
                 </Flex>
             </Text>
